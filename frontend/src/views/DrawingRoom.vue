<template>
  <div class="chat-container">
    ...
    <!-- 中间画布区域（桌面端，始终显示画布，且不在游戏模式） -->
    <div v-if="roomId && !isMobile && !showGamePanel" class="drawing-area">
      <DrawingPanel
    ...
    <div class="right-chat" :class="{
      'with-drawing': roomId && !showGamePanel,
      ...
      <div class="chat-main" @click="hideMobileNavbar(); hideMusicMenu()">
        ...
        <template v-else>
          ...
          <div v-if="roomId && isMobile" class="drawing-panel mobile-drawing-panel">
            <DrawingPanel
              :currentDrawer="currentDrawer"
              :username="username"
              :isConnected="isConnected"
...
export default {
  ...
  data () {
    return {
      ...
      sidebarCollapsed: true, // 左侧菜单折叠状态（默认隐藏，避免初始黑闪）
      musicConfig: {},
      showMusicMenu: false,
      isPlaying: false,
      currentMusicId: null,
  ...
  methods: {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (envelope.game && this.GameMessage) {
            const err = this.GameMessage.create({
              type: this.GameMessage.Type.ERROR,
              error: { message: '当前房间类型不支持游戏功能' }
            })
            const errEnvelope = this.WsEnvelope.create({ game: err })
            const errBuf = this.WsEnvelope.encode(errEnvelope).finish()
            this.ws.send(errBuf)
            // 非游戏房间直接返回，不再继续处理本条消息
            return
          }
    ...
    switchRoom (roomId) {
      if (roomId !== this.roomId) {
        ...
        this.$router.push(`/room/drawing/${roomId}`)
        // 移动端切换房间后隐藏导航栏
        if (this.isMobile) {
