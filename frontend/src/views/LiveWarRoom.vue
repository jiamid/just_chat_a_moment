    ...
    <div class="right-chat" :class="{
      ...
      <div class="chat-main" @click="hideMobileNavbar(); hideMusicMenu()">
        ...
        <template v-else>
          ...
          <div v-if="showGamePanel && isMobile && roomId" class="game-panel-mobile">
            <GamePanel
              :gameState="gameState"
              :gameOverInfo="gameOverInfo"
              :isConnected="isConnected"
...
export default {
  ...
  async mounted () {
    ...
    if (this.roomId) {
      ...
      // 桌面端默认展开游戏面板，移动端默认隐藏（可通过按钮切换）
      if (!this.isMobile && !this.showGamePanel) {
  ...
  methods: {
    ...
    switchRoom (roomId) {
      if (roomId !== this.roomId) {
        ...
        this.$router.push(`/room/live_war/${roomId}`)
        // 移动端切换房间后隐藏导航栏
        if (this.isMobile) {
